...
export default function ClassReportCards() {
  ...
  const [grades, setGrades] = useState<Record<string, Record<number, any>>>({});
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        gradesSnap.docs.forEach(d => {
          ...
          gradesMap[data.studentId][data.bimester] = data;
        });
        setGrades(gradesMap);
        ...
        {students.map((student, index) => (
          <div key={student.id} className={index < students.length - 1 ? "break-after-page mb-16 print:mb-0" : ""}>
            ...
            <table className="min-w-full border-collapse border border-gray-400">
              ...
              <tbody>
                {bimesters.map(b => {
                  const g = (grades[student.id] && grades[student.id][b]) || {};
